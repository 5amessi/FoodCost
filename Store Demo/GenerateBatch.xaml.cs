...
namespace Food_Cost
{
    ...
    public partial class GenerateBatch : UserControl
    {
                    ...
                    else
                    {
                        ...
                        if (reader2.HasRows == true)
                        {
                            while (reader2.Read())
                            {
                                ...
                                {
                                    Qty = (((Convert.ToDouble(reader2["Qty"])) - (((Convert.ToDouble(QtyofRecipetxt.Text)) * (Convert.ToDouble(reader["Qty"]))) / Convert.ToDouble(BaseWeight))) * Convert.ToDouble(YeildofQty) / 100);
                                    if (Qty < 0)
                                    {
                                        boooolCHeck = false;
                                    }
                                    else
                                    {
                                        ...
                                        RecipeItemDData[CountofRecipeItemData, 2] = ((Convert.ToDouble(QtyofRecipetxt.Text)) * (Convert.ToDouble(reader["Qty"])) * (Convert.ToDouble(YeildofQty) / 100) * (Convert.ToDouble(reader2["Current_Cost"]))).ToString();
                                ...
                                {
                                    Qty = (((Convert.ToDouble(reader2["Qty"])) - (((Convert.ToDouble(QtyofRecipetxt.Text)) * (Convert.ToDouble(reader["Qty"]) / Convert.ToDouble(secondWeight))) / Convert.ToDouble(BaseWeight))) * Convert.ToDouble(YeildofQty) / 100);
                                    if (Qty < 0)
                                    {
                                        boooolCHeck = false;
                                    }
                                    else
                                    {
                                        RecipeItemDData[CountofRecipeItemData, 0] = reader["Item_Code"].ToString();
                                        RecipeItemDData[CountofRecipeItemData, 1] = ((float.Parse(reader["Qty"].ToString()) / float.Parse(BaseWeight)) * float.Parse(QtyofRecipetxt.Text)).ToString();
                                        RecipeItemDData[CountofRecipeItemData, 2] = ((Convert.ToDouble(QtyofRecipetxt.Text)) * (Convert.ToDouble(reader["Qty"])) * (Convert.ToDouble(secondWeight)) * (Convert.ToDouble(YeildofQty) / 100) * (Convert.ToDouble(reader2["Current_Cost"]))).ToString();
                                        RecipeItemDData[CountofRecipeItemData, 3] = Qty.ToString();
                                        CountofRecipeItemData++;
                                    }
                                }
        ...
        public void UpddateQty()
        {
            ...
            {
                con.Open();
                string E = string.Format("Select Top(1)Generate_ID From GenerateRecipe_tbl where Generate_ID like '{0}%' ORDER BY Generate_ID DESC", Classes.IDs);
                cmd = new SqlCommand(E, con);
                if (cmd.ExecuteScalar() == null)
                {
                    valOfGenerate = Classes.IDs + "0000001";
                }
                else
                {
                    valOfGenerate = "0" + (Int64.Parse(cmd.ExecuteScalar().ToString()) + 1).ToString();
                }
                con.Close();
            }
            catch { }
            ...
            try
            {
                ...
                {
                    s = string.Format("Update RecipeQty SEt Qty=Qty+{1},Price=(((Qty*Price)+({1}*{4}))/Qty+{1}) Where Recipe_ID={0} AND Resturant_ID={2} AND Kitchen_ID={3}", valofRecipe, Convert.ToDouble(QtyofRecipetxt.Text), valofStore, ValOfKitchen, CostOfrecipe);
                    cmd = new SqlCommand(s, con);
                    cmd.ExecuteNonQuery();
                }
            }
            ...
            try
            {
                ...
                for (int i = 0; i < CountofRecipeItemData; i++)
                {
                    string s = string.Format("Insert into GenerateRecipe_Items(Generate_ID,Item_ID,ItemQty,Cost,Net_Cost,Current_Qty) Values ('{0}','{1}','{2}','{3}','{4}','{5}')", valOfGenerate, RecipeItemDData[i, 0], RecipeItemDData[i, 1], RecipeItemDData[i, 2], (Convert.ToDouble(RecipeItemDData[i, 1]) * Convert.ToDouble(RecipeItemDData[i, 2])), RecipeItemDData[i, 3]);
                    cmd = new SqlCommand(s, con);
                    cmd.ExecuteNonQuery();
                }
                for (int i = 0; i < CountofRecipesData; i++)
                {
                    string s = string.Format("Insert into GenerateRecipe_Items(Generate_ID,reccipe_ID,RecipeQty,Cost,Net_Cost,Current_Qty) Values ('{0}','{1}',{2},'{3}','{4}','{5}')", valOfGenerate, RecipesData[i, 0], RecipesData[i, 1], RecipesData[i, 2], (Convert.ToDouble(RecipesData[i, 1]) * Convert.ToDouble(RecipesData[i, 2])), RecipesData[i, 3]);
                    cmd = new SqlCommand(s, con);
                    cmd.ExecuteNonQuery();
                }
            }
