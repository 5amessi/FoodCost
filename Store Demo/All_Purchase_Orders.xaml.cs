        ...
        private void PO_DGV_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            if (main.GridMain.Children[0].GetType().Name == "PurchaseOrder")
            {
                try
                {
                    ...
                    purchaseOrder.Statustxt.Text = reader["Status"].ToString();

                    reader.Close();
                    cmd = new SqlCommand(string.Format("select Status From RO Where Type='Recieve_Purchase' and Transactions_No={0}", dt.Rows[PO_DGV.SelectedIndex]["Serial"]), con);
            ...
            else if (main.GridMain.Children[0].GetType().Name == "RecieveOrder")
             {
                ...
                float from_rest_Qty = 0;
                float from_rest_Cost = 0;
                float to_rest_Qty = 0;
                float to_rest_Cost = 0;
                if (recieveOrder.TabControl.SelectedIndex == 1)
                {
                    ...
                    recieveOrder.ManualROKitchen.IsEnabled = true;
                    recieveOrder.DeliveryROKitchen.IsEnabled = true;
                    recieveOrder.DeliveryROKitchenTime.IsEnabled = true;
                    recieveOrder.CommentKitchen.IsEnabled = true;
                    ...
                    try
                    {
                        ...
                        recieveOrder.FromKitchenKitchentxt.Text = row["From_Kitchen"].ToString();
                        recieveOrder.ToKitchenKitchentxt.Text = row["To_Kitchen"].ToString();
                    }
                    catch (Exception ex)
                    {
                    ...
                    Dat.Columns.Add(recieveOrder.FromKitchenKitchentxt.Text + " total Cost");
                    Dat.Columns.Add(recieveOrder.ToKitchenKitchentxt.Text + " Qty");
                    Dat.Columns.Add(recieveOrder.ToKitchenKitchentxt.Text + " Unit Cost");
                    Dat.Columns.Add(recieveOrder.ToKitchenKitchentxt.Text + " total Cost");
                    try
                    {
                        con.Open();
                        string M = "SELECT Item_ID,Qty,Cost FROM Requests_Items where Request_ID=" + CodeofRO ;
                    ...
                    recieveOrder.ItemKitchenTransferDGV.DataContext = Dat;
                    float Total_Price = 0;
                ...
                else if (recieveOrder.TabControl.SelectedIndex == 2)
                {
                    ...
                    recieveOrder.ManualROInter.IsEnabled = true;
                    recieveOrder.DeliveryROInter.IsEnabled = true;
                    recieveOrder.DeliveryROInterTime.IsEnabled = true;
                    recieveOrder.CommentRoInter.IsEnabled = true;
                    DataTable Dat = new DataTable();
                    try
                    {
                        con.Open();
                        ...
                        recieveOrder.ToKitchenIntertxt.Text = row["To_Kitchen"].ToString();
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.ToString());
                    ...
                    Dat.Columns.Add(recieveOrder.FromKitchenIntertxt.Text + " total Cost");
                    Dat.Columns.Add(recieveOrder.ToKitchenIntertxt.Text + " Qty");
                    Dat.Columns.Add(recieveOrder.ToKitchenIntertxt.Text + " Unit Cost");
                    Dat.Columns.Add(recieveOrder.ToKitchenIntertxt.Text + " total Cost");
                    try
                    {
                        con.Open();
                        string M = "select Item_ID,Qty,Cost FROM Requests_Items where Request_ID=" + CodeofRO ;
                    ...
                    Dat.Columns["Received"].ReadOnly = false;
                    Dat.Columns["Qty"].ReadOnly = false;
                    recieveOrder.ItemRoInterDGV.DataContext = Dat;
                    float Total_Price = 0;
                ...
                else if(recieveOrder.TabControl.SelectedIndex == 4)
                {
                    ...
                    {
                        con.Open();
                        string s = string.Format("select Type from RO where (Type='Transfer_Resturant' OR Type='Transfer_Kitchen' ) and PO_No='{0}'", recieveOrder.TransferNumberInterTxt.Text);
                        SqlCommand cmd = new SqlCommand(s, con);
                    ...
                    {
                        con.Open();
                        string M = "SELECT Item_ID,Qty,Cost FROM Requests_Items where Request_ID=" + recieveOrder.Serial_Request_NO.Text;
                        SqlCommand cmd = new SqlCommand(M, con);
                        SqlDataReader reader = cmd.ExecuteReader();
                        while (reader.Read())
                        {
                    ...
                    {
                        recieveOrder.Serial_Request_NO.IsEnabled = false;
                        recieveOrder.Request_NO.IsEnabled = false;
                    ...
                    {
                        recieveOrder.Serial_Request_NO.IsEnabled = true;
                        recieveOrder.Request_NO.IsEnabled = true;
            ...
            else if (main.GridMain.Children[0].GetType().Name == "Transfer_Kitchens")
            {
                ...
                try
                {
                    ...
                    reader.Close();
                    con2.Open();
                    string W = string.Format("select Item_ID as Code,(select [Manual Code] from Setup_Items where Code= Item_ID) as Manual_Code,(select Name from Setup_Items where Code = Item_ID) as Name,(select Name2 from Setup_Items where Code = Item_ID) as Name2, Qty from Transfer_Kitchens_Items where Transfer_ID = '{0}' order by Serial", transferKitchen.transfer_No.Text);
                    ...
                    while (reader.Read())
                    {
                        try
                        {
                            ...
                            using (cmd2 = new SqlCommand(string.Format("select Qty,Current_Cost from Items where ItemID = '{0}' and RestaurantID = (select Code from Setup_Restaurant where Name = '{1}') and KitchenID = (select Code from Setup_Kitchens where Name = '{2}') union all select Qty, Current_Cost from Items where ItemID = '{0}' and RestaurantID = (select Code from Setup_Restaurant where Name = '{1}') and KitchenID = (select Code from Setup_Kitchens where Name = '{3}')", reader["Code"], transferKitchen.Resturant.Text, transferKitchen.From_Kitchen.Text, transferKitchen.To_Kitchen.Text), con2))
                            {
                                ...
                                float from_rest_Qty = float.Parse(reader2["Qty"].ToString());
                                float from_rest_Cost = float.Parse(reader2["Current_Cost"].ToString());

                                reader2.Read();
                                float to_rest_Qty = float.Parse(reader2["Qty"].ToString());
                                float to_rest_Cost = float.Parse(reader2["Current_Cost"].ToString());
                        ...
                        reader2.Close();
                        dt.Rows.Add(reader["Code"], reader["Manual_Code"], reader["Name"], reader["Name2"], reader["Qty"], FromKitchenQty, FRomKitchenCost, FromKitchenTotal, ToKitchenQty, ToKitchenCost, ToKitchenTotal);
                    }
            ...
            else if (main.GridMain.Children[0].GetType().Name == "Transfer_Resturant")
            {
                ...
                try
                {
                    ...
                    con2.Open();
                    string W = string.Format("select Item_ID as Code,(select [Manual Code] from Setup_Items where Code= Item_ID) as Manual_Code,(select Name from Setup_Items where Code = Item_ID) as Name,(select Name2 from Setup_Items where Code = Item_ID) as Name2, Qty from Transfer_Kitchens_Items where Transfer_ID = '{0}' order by Serial", transfer_Resturant.transfer_No.Text);
                    cmd = new SqlCommand(W, con);
                    reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
